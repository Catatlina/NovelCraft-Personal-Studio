...
# local runs use localhost and CI/Compose keep their explicit service host while
# only forcing the isolated database 15.
def _isolated_test_redis_url() -> str:
    explicit = os.getenv("NOVELCRAFT_TEST_REDIS_URL", "").strip()
    if explicit:
        return explicit
    from urllib.parse import urlsplit, urlunsplit
    # Local runs normally expose Redis on localhost.  Docker/CI deployments
    # provide an explicit REDIS_URL (usually the service name ``redis``), so
    # this default does not change those environments.
    parts = urlsplit(os.getenv("REDIS_URL", "redis://localhost:6379/0"))
