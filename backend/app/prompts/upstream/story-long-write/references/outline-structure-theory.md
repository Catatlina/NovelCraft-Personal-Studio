...
一级结构是一个完整单元剧：一个矛盾从起到爆发收束，约 1.5-3 万字 / 数章。拆对标书拆出来的「剧情线 / 章节摘要 / 爽点循环」就是它的一级结构序列；自己卷纲里的「爽点节奏周期」也是一级结构。所以对标节奏不是另起炉灶——是把对标书的一级结构序列，保留情绪与功能、换素材，搬进自己的卷纲。
...
1. **选段**：在主对标里圈出本卷要对标的一级结构序列。优先读 `剧情/节奏.md`（爆发密度 / 爽点循环）；没有就用 `章节/*_摘要.md`（每个情节点带「类型」「基调」标签）+ `拆文报告.md`（剧情条的章节范围 / 结构分布）——后者足够还原，旧拆文库常无 `节奏.md`。本卷对标几条一级结构 ≈ 本卷字数 ÷ 1.5-3 万。
2. **标关键点**：看对标这几个一级结构在 1/4、中点、3/4 各放了什么关键情节，判为逆转 / 转折 / 激励。摘要里「类型=转折点」的情节点 + 基调突变（如温馨→压抑）就是关键点的确定性锚，不靠凭感觉。
3. **换素材**：保留对标的情绪链和功能位，把对象、场景、事件全换成自己的，禁止照搬具体桥段。功能位清单优先取 `剧情/情绪模块.md` 可替换项；没有就用摘要的「基调」序列 + 剧情条的「桥段标签」当替代。
...
5. **排章 + 落卷纲**：把换好（并修正）的关键情节落到本卷相同相对位置——这里的 1/4·中点·3/4 是**卷尺度**（1/4≈总章×0.25、中点×0.5、3/4×0.75）；卷里有几条一级结构时，每条一级结构内部另有自己的铺垫→逆转→收尾，别和卷尺度的关键点混为一谈。关键点之间用普通推进章过渡，每个关键点之后按需插一章呼吸 / 关系章；写进卷纲「对标结构坐标」与情绪弧线的章节定位列，细纲承接时每章标章节定位。
