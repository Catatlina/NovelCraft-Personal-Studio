...
1. 段落炸碎：每段 1-3 句话，描写/对话/动作交替穿插，绝不允许一整段全是描写
2. 口语化：删除「然而/因此/不仅…而且/与此同时/总而言之」等书面连接词；「说」换成具体动作（笑道/叹了口气/头也不抬/手上的动作没停）；加入「说实话/说白了/没办法/谁让」等口语插入语；一个句子里「的」不超过 2 个
3. 人味注入：每 3-5 段插一句内心独白或叙述者吐槽；对话要像人话不像剧本，带上说话时的表情、动作、语气；数字不写太精确（「大概十几分钟」而非「15 分钟」）
4. 节奏控制：紧张处句子更短、段落更短；每个场景至少一处让人「有画面感」的细节——用具体动作或感官，不用成语概括
5. 删 AI 痕迹：删「通过…/从而…/进而…」公文句式和「不可否认/显而易见/众所周知」措辞；情绪不直接说出来（「他很愤怒」→ 砸了杯子/手在抖/呼吸变重）；严禁「命运的齿轮/心猛地一沉/眼神复杂/踏上新的旅程」等套话与「这一切都说明/他终于明白」章末总结体

【AI 味黑名单——以下短语每章最多出现 0 次，出现即替换】
- 「忽然/忽然间/忽然发现/忽然想到」→ 删掉，直接写动作转折（"门开了"而非"忽然，门开了"）
- 「心里咯噔一下/心里一沉/心中一凛」→ 换成具体反应（手抖了/停住了/后退一步/瞳孔缩了）
- 「深吸一口气/长出一口气」→ 换成具体动作（蹲下来/转过身/把烟点上/攥紧了拳头）
- 「嘴角微微上扬/嘴角抽了抽/嘴角勾起一抹弧度」→ 换成不那么套路的反应（笑了/嗤了一声/鼻子哼了一下）
- 「忍不住/不由得/不禁/情不自禁」→ 删掉，直接写动作（"我笑了"而非"我忍不住笑了"）
- 「下意识/本能地/条件反射」→ 删掉或换成具体动作
- 「仿佛/似乎/好像/宛如/犹如」→ 全章合计不超过 2 次，超过的全部删掉
- 「顿时/随即/霎时/蓦然」→ 换成「就/直接/一下子」等口语
- 「一时间/刹那间/一瞬间/转瞬间」→ 删掉，用短句本身表达速度
- 「心想/心里暗想/暗自思忖」→ 直接写内心独白（"这老头怕不是有病吧"而非"他心想，这老头怕不是有病吧"）

写完自己读一遍：如果读起来像作文，就再改一遍——目标是真人老作者随手写的口水感，不讲究、不端着，但读着舒服"""
...
【爆款书名范式——务必参照这些真实案例的写法】
- 第一人称/口语/吐槽，自带记忆点：《我真没想重生啊》《都重生了谁谈恋爱啊》《癫，都癫，癫点好啊》《重生：谁也别想安排我》
- 具体时代符号而非裸年份：《1979黄金时代》《重生70整顿亲情》《重生野性时代》（用「黄金时代/千禧/诺基亚/网吧/房改/下岗」等时代符号，不要直接写「2010」这类干年份）
...
要求：8-12 卷覆盖百万字、前 30 章单章级详细、每卷有高潮和钩子。"""),
    ...
    ("bootstrap.gen_chapter1", "3.4.0", "deepseek",
...
【篇幅硬要求】保留原文篇幅：润色后总字数不得低于 2000 字，不得压缩、不得总结、不得删减情节；若原文较长，输出需与之相当

""" + DEAI_IRON_RULES + """

【排版硬要求】必须用空行分隔每个段落（段落之间用换行隔开），每段 1-3 句话；严禁把全部内容写成一整段没有换行的文本。输出 JSON: {"text":"润色后的文本"}"""),

    ("editor.rewrite", "3.2.0", "deepseek",
     """请按要求改写以下文本。
...
【篇幅硬要求】保留原文篇幅：改写后总字数不得低于 2000 字，不得压缩、不得总结、不得删减情节；若原文较长，输出需与之相当
...
输出 JSON: {"titles":["标题1","标题2","标题3"]}"""),
    ...
    ("bootstrap.plan_idea", "1.3.0", "deepseek",
...
输出 JSON: {"passed":false,"score":80,"matched_requirements":["已保留的要求1","已保留的要求2","已保留的要求3"],"contradictions":["原需求为X，规划却为Y"],"omissions":["规划遗漏Z"]}"""),
    ...
    ("bootstrap.regenerate_titles", "1.2.0", "deepseek",
...
2. 书名 6-14 字，像真人在榜单发的书名。必须从「人物状态、情绪、时代符号、反差/悬念」切入，禁止从「设定/金手指/年代」切入。绝对禁止书名直接出现「AI、人工智能、2010、重生、穿越、系统、算法、科技帝国、比特币、区块链、元宇宙」等题材关键词（除非已被彻底口语化/吐槽化）；禁止《重生2010：我的AI笔记本》《我的AI能预知未来》《重生之算法为王》《带着AI去2010》这类关键词堆砌/SEO 式书名；禁止「我的XX能XX」「XX之XX为王/称霸/无敌」「从XX开始」万能模板。爆款范式参考：《我真没想重生啊》《都重生了谁谈恋爱啊》《1979黄金时代》《都重生了才告诉我全家是反派》《我不是戏神》
3. 禁止用与内容无关的夸张标题，禁止机械套用“开局/无敌/震惊/系统逼我/废柴/逆袭/无双”等模板
4. 只提供候选，不替用户决定
...
输出 JSON: {"story_arcs":[{"name":"第一次创业","goal":"主角从零建立公司","start_state":"失业负债","end_state":"公司走上正轨","participants":["主角","合伙人"],"core_conflict":"资金与对手打压","key_events":["凑启动资金","首个订单"],"payoff_points":["首笔大单落地"],"foreshadowing_refs":[],"outcome_impact":"奠定后续资本局","status":"planning","chapter_range":[5,40]}]}"""),
    ...
    ("bootstrap.write_chapter_draft", "1.2.0", "deepseek",
...
6. 正文不少于 2000 字，建议 2200-3200 字，8-18 个叙事段落
...
6. 不得缩写、摘要或删除细节；逐段等量改写，目标长度为原文的 90%–110%
