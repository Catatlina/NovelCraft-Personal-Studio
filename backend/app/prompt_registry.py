...
要求：8-12 卷覆盖百万字、前 30 章单章级详细、每卷有高潮和钩子。"""),
    ...
    ("bootstrap.gen_chapter1", "3.1.0", "deepseek",
...
7. 正文不少于 2000 字，建议 2200-3200 字，8-15 个叙事段落；去 AI 味：避免过于工整的句式、章末总结体、套话，多用口语停顿、具体动作与对话推进
...
【篇幅与去味硬要求】
- 保留原文篇幅：润色后总字数不得低于 2000 字，不得压缩、不得总结、不得删减情节；若原文较长，输出需与之相当
- 必须去除 AI 味：避免过于工整的句式、章末总结体、套话，增加口语停顿、具体动作与对话

【排版硬要求】必须用空行分隔每个段落（段落之间用换行隔开），每段 1-3 句话；严禁把全部内容写成一整段没有换行的文本。输出 JSON: {"text":"润色后的文本"}"""),

    ("editor.rewrite", "3.1.0", "deepseek",
     """请按要求改写以下文本。
...
【篇幅与去味硬要求】
- 保留原文篇幅：改写后总字数不得低于 2000 字，不得压缩、不得总结、不得删减情节；若原文较长，输出需与之相当
- 必须去除 AI 味：避免过于工整的句式、章末总结体、套话，增加口语停顿、具体动作与对话
...
输出格式：用空行分隔段落；每个段落 1-3 句话。输出 JSON: {"text":"去AI味后的文本"}"""),
    ...
    ("narrative.gen_next_chapter", "3.3.0", "deepseek",
...
输出 JSON: {"story_arcs":[{"name":"第一次创业","goal":"主角从零建立公司","start_state":"失业负债","end_state":"公司走上正轨","participants":["主角","合伙人"],"core_conflict":"资金与对手打压","key_events":["凑启动资金","首个订单"],"payoff_points":["首笔大单落地"],"foreshadowing_refs":[],"outcome_impact":"奠定后续资本局","status":"planning","chapter_range":[5,40]}]}"""),
    ...
    ("bootstrap.write_chapter_draft", "1.1.0", "deepseek",
...
6. 正文不少于 2000 字，建议 2200-3200 字，8-18 个叙事段落；去 AI 味：避免过于工整的句式、章末总结体、套话，多用口语停顿、具体动作与对话推进
