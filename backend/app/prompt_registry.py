...
PROMPT_SEEDS = [
    ...
    ("bootstrap.gen_titles", "3.1.0", "deepseek",
     """你是资深网文主编，深谙番茄小说、起点中文网的爆款书名套路。请基于以下设定生成 5 个「像真人在榜单上发的」小说书名，不要写关键词堆砌的机器标题。
...
- 第一人称/口语/吐槽，自带记忆点：《我真没想重生啊》《都重生了谁谈恋爱啊》《癫，都癫，癫点好啊》
- 具体时代符号而非裸年份：《1979黄金时代》《重生70整顿亲情》《重生野性时代》（用「黄金时代/千禧/诺基亚/网吧/房改/下岗」等时代符号，不要直接写「2010」这类干年份）
- 反差/冲突/悬念：《都重生了才告诉我全家是反派》《重生：谁也别想安排我》《系统赋我长生，我熬死了所有人》
- 人物状态/情绪：《我不是戏神》《我在精神病院学斩神》

【硬性要求】
1. 像真实榜单书名，有网感、有钩子，读起来像人取的，不像机器生成的 SEO 标题
2. 用反差、悬念、口语吐槽、具体时代符号制造记忆点；禁止把「重生/穿越/系统/AI/年份」等关键词平铺堆砌成标题
3. 字数 6-14 字为主，可用逗号、冒号制造节奏，但不得写成一句话简介
4. 每个书名附一句「为什么能爆」（点明踩中哪条爆款范式）
5. 禁止烂俗模板词（废柴/逆袭/无双/天才/横扫/称霸）；不要把题材关键词当书名主体
...
输出格式：用空行分隔段落；每个段落 1-3 句话。输出 JSON: {"text":"去AI味后的文本"}"""),
    ...
    ("narrative.gen_next_chapter", "3.2.0", "deepseek",
...
3. 保持人物性格一致——参考人物档案
4. 伏笔要么推进要么回收——不能种了不管
5. 正文不得低于 3000 字；续写或重写时也必须达到 3000 字，不得因前文短而缩短
6. 结尾留钩子，但不要用"新的篇章开始了"这类总结句
7. 必须写小说正文：用动作、对话、场景细节和心理微反应推进，不写说明文、计划书、创作建议
8. 如果是重写，保留章节序号和核心剧情目标，但要显著改善冲突密度、画面感、口语节奏和去 AI 味；若 $review_feedback 含「字数不足」，必须扩写至 3000 字以上，不得删减情节，并逐条修复反馈列出的问题
9. 去 AI 味：避免过于工整的句式、章末总结体、面面俱到；多用口语停顿、具体动作和对话推进
10. 排版要求：每段只包含 1-3 句话，按动作、对话、心理转折或场景切换频繁换段；禁止大段密集说明文

输出 JSON: {"chapter":{"title":"第N章 创意标题","body":["段落一","段落二",...]}}
body 至少 12 段，建议 15-25 段；每段 1-3 句；总字数不得低于 3000 字；低于 3000 字视为失败。"""),
...
输出 JSON: {"titles":["标题1","标题2","标题3"]}"""),
    ...
    ("bootstrap.plan_idea", "1.1.0", "deepseek",
...
4. title_candidates：生成 3-10 个书名候选（默认 5 个），像真人在榜单发的书名，参照爆款范式——第一人称/口语吐槽（《我真没想重生啊》）、具体时代符号非裸年份（《1979黄金时代》用「黄金时代/千禧/诺基亚/房改」而非「2010」）、反差悬念（《都重生了才告诉我全家是反派》）、人物状态情绪（《我不是戏神》）；禁止把「重生/穿越/系统/AI/年份」平铺堆砌成标题；6-14 字为主；避免烂俗模板词（废柴/逆袭/无双）；如果有 suggested_title，只把它作为一个候选参考，不得直接替用户确认
...
输出 JSON: {"passed":false,"score":80,"matched_requirements":["已保留的要求1","已保留的要求2","已保留的要求3"],"contradictions":["原需求为X，规划却为Y"],"omissions":["规划遗漏Z"]}"""),
    ...
    ("bootstrap.regenerate_titles", "1.1.0", "deepseek",
...
2. 书名 6-14 字，像真人在榜单发的书名，参照爆款范式（第一人称吐槽/具体时代符号/反差悬念/人物状态情绪，参考《我真没想重生啊》《1979黄金时代》《都重生了才告诉我全家是反派》《我不是戏神》），禁止把「重生/穿越/系统/AI/年份」平铺堆砌成标题
