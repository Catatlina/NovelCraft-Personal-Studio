...
4. 禁止破折号堆砌：「——」每章最多 2 次，超过的用逗号/句号/省略号替代
5. 省略号统一用「……」（6 点），不用「...」「。。。」「…」（3 点）
...
9. 一个句子里「的」不超过 2 个
10. 每 3-5 段插一句内心独白或叙述者吐槽（第一人称尤其重要）
11. 数字不写太精确：「大概十几分钟」而非「15 分钟」

=== 第三层：弱化副词（最大 AI 痕迹）===
12. 以下副词每千字合计不超过 3 个，超过全部删掉：
...
=== 第五层：AI 高频词黑名单（每章 0 次）===
- 「忽然/忽然间」→ 删掉，直接写动作转折（"门开了"而非"忽然，门开了"）
- 「心里咯噔一下/心里一沉/心中一凛」→ 具体反应（手抖了/停住了/后退一步）
- 「深吸一口气/长出一口气」→ 具体动作（蹲下来/转过身/把烟点上/攥紧拳头）
- 「嘴角微微上扬/嘴角抽了抽/嘴角勾起一抹弧度」→ 笑了/嗤了一声/鼻子哼了一下
- 「忍不住/不由得/不禁/情不自禁」→ 删掉，直接写动作
- 「下意识/本能地/条件反射」→ 删掉或换具体动作
- 「顿时/随即/霎时/蓦然」→ 换成「就/直接/一下子」
- 「一时间/刹那间/一瞬间/转瞬间」→ 删掉，用短句表达速度
- 「心想/心里暗想/暗自思忖」→ 直接写内心独白
- 「仿佛/似乎/好像/宛如/犹如」→ 全章合计不超过 2 次
- 「沉声道/淡淡地说/冷冷地说」→ 删掉说话方式标签，用动作替代
- 「脸色一变/身形一顿/瞳孔一缩」→ 写具体反应（他停住了/他的手僵在半空）
- 「目光如炬/目光深邃/眼神复杂」→ 写眼睛看向哪里（他盯着门缝/她没看他）
- 「映入眼帘/只见/此时此刻」→ 删掉，直接写看到的东西
- 「不难看出/由此可见/事实上/值得注意的是」→ 全部删掉（论文句式）

=== 第六层：正式语体清除 ===
- 「于是乎/诚然/一方面…另一方面…/与此同时/从而/因而/进而」→ 全部删掉或改口语
- 「不可否认/显而易见/众所周知/毋庸置疑」→ 全部删掉
- 「通过…方式/以…为契机/在…的基础上」→ 公文句式，全部删掉
