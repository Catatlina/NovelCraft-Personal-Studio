...
_FEEDBACK_INTENSITY_RULES: dict[str, Any] = {
    ...
    "crowd_feedback_design": {
        ...
        "principles": [
            ...
            # 番茄爽文加码：夸张反馈原则
            "夸张原则：反馈要足够夸张，足够有画面感，让读者看得爽",
            "反差原则：之前越看不起主角，之后越震惊，反差越大越爽",
        ],
        "anti_patterns": [
            "千篇一律的'众人哗然'（所有人反应都一样）",
            "只有旁白描述，没有具体动作和细节",
...
_VILLAIN_DESIGN_RULES: dict[str, Any] = {
    "cross_level_face_slap": {
        ...
        # 番茄爽文加码：打脸四部曲强化版
        "face_slap_structure": [
            "反派嚣张挑衅（必须够欠揍，让读者恨得牙痒痒）",
            "众人不看好主角（压低预期，所有人都觉得主角必输）",
            "主角隐藏实力（蓄势，读者知道主角牛逼，但剧中人不知道）",
            "主角出手碾压（爆发，必须彻底碾压，不能五五开）",
            "全场哗然/反派崩溃（反馈，必须夸张，众人石化、鸦雀无声）",
            "更大的反派出现（新压力，打完小的来大的）",
        ],
        # 番茄爽文加码：反派嚣张度要求
        "villain_arrogance": {
            "label": "反派嚣张度要求",
            "description": "反派必须足够嚣张、足够欠揍，打脸才够爽",
            "requirements": [
                "反派必须当面羞辱主角，不能只是背后使坏",
                "反派必须提出过分要求（下跪、道歉、赔钱、滚出去等）",
                "反派必须有狗腿子附和，增加嚣张氛围",
                "反派必须看不起主角，觉得主角就是个废物",
                "反派必须放狠话，比如'今天你要是能赢我，我就吃屎'",
            ],
            "early_stage": "前10章的反派必须特别嚣张，特别欠揍，让读者一看就想抽他",
        },
        # 番茄爽文加码：打脸狠度要求
        "face_slap_intensity": {
            "label": "打脸狠度要求",
            "description": "打脸必须够狠、够彻底，不能留余地",
            "requirements": [
                "必须当面打脸，不能背后打脸",
                "必须彻底碾压，不能五五开，不能险胜",
                "反派必须崩溃、失态、颜面扫地",
                "反派必须付出惨重代价（赔钱、丢面子、失去地位等）",
                "旁观者必须震惊，反差越大越好",
            ],
            "early_stage": "前10章的打脸必须特别狠，特别彻底，让读者看得特别爽",
        },
...
_MECHANIC_RULES: dict[str, dict[str, Any]] = {
    ...
    "invincible_opening": {
        "label": "无敌开局/扮猪吃虎",
        # 番茄爽文加码：无敌开局爽感拉满
        "payoff_intensity": "peak",        # 无敌开局爽感最强
        "loop": "隐藏底牌→制造误判→选择亮牌时机→局面反转→对手与旁观者反馈→暴露更高层压力",
        "guard": "不能连续靠隐藏实力重复同一种打脸；必须升级对手、场景和亮牌代价。",
        # 番茄爽文加码：开局碾压设计导向强化
        "opening_domination": True,        # 开局碾压级优势
        "opening_showcase_chapter": 1,     # 加码：第1章就展示碾压级实力
        "early_chapter_no_cost": True,     # 加码：前10章先爽再说，代价和限制往后放
        "early_cost_delay_chapters": 10,   # 加码：前10章不强调代价，第10章后再逐步引入限制
    },
    "anti_trope": {
        "label": "反套路/弱金手指",
        # P0-5 质量整改：金手指爽感强度
