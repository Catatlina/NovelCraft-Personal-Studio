...
_ANTI_TEMPLATE_RULES: tuple[str, ...] = (
    ...
    "后台节拍、因果账本和质量规则只服务于写作选择，不得在正文中变成步骤清单、操作说明或作者讲解。",
    "默认不使用‘第一步/第二步/第三步’或‘首先/其次/最后’串联叙事；只有人物确实制定计划且会改变行动时才保留必要片段。",
)
...
def render_readability_plan(plan: dict[str, Any] | None, *, compact: bool = False) -> str:
    ...
    lines = [
        ...
        "叙事质感：不要把节拍表复述成正文；让读者从动作、对白、物件和后果自行拼出因果，不连续使用解释性桥句。",
    ]
    if not compact:
