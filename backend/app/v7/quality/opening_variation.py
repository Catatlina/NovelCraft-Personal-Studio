...
_OBJECT_RE = re.compile(
    r"(?:手机|玉佩|令牌|钥匙|账本|合同|文件|屏幕|信封|药瓶|刀|剑|戒指|手表|门锁|门缝|门面|铜门|木门|灯|光线|微光|裂痕|字迹|灰尘|箱子).{0,20}(?:裂|亮|震|响|掉|翻|停|显|渗|开|断|出现|变|闪|暗|躺|悬|浮|摆)"
    r"|(?:[这那一某]?(?:卷|把|枚|块|张|封|件|个|本|道|片|根|柄|盒|袋|团|幅|扇|盏|只|东西)).{0,24}(?:裂|亮|震|响|掉|翻|停|显|渗|开|断|出现|变|闪|暗|躺|悬|浮|摆|散|裹)"
)
_ENVIRONMENT_RE = re.compile(
    r"(?:雨|雾|风|雪|潮气|热浪|冷气|天光|灯光|地面|楼道|院子|街道|海面|山谷).{0,20}(?:压|卷|散|落|亮|暗|晃|漫|逼|涌|变)"
)
_ACTION_RE = re.compile(
    r"(?:抬手|转身|推开|推门|进门|冲出|拔出|按住|抓住|扣下|抵住|迈步|迈进|抬脚|走近|靠近|拎起|握紧|扑|躲|拦|挡|撕|砸|掏出|取出|站起|跪|回头|走向|往.{0,8}走|上楼|下楼|决定|开口|划|扫|扫过|擦|拂过|踩|弯腰|俯身|抬头|伸手|捡起|拾起|掀|扯|提|拉|拽|踢|翻|掩|靠|贴|收回|压住|放下|抓起|蹲下|退后|退开|直起|停住|停下|顿住|顿了一下|摸到|碰到|挪|移|撬|踮)"
)
