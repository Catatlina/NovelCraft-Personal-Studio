...
export function Layout({ tab, setTab, title, children }: {
  ...
  const commands = [
    { id: "overview", label: "概览", action: () => setTab("overview") },
    { id: "dashboard", label: "工作台", action: () => setTab("dashboard") },
    { id: "wizard", label: "灵感创作", action: () => setTab("wizard") },
    { id: "ranking", label: "扫榜选书", action: () => setTab("ranking") },
    { id: "library", label: "书库管理", action: () => setTab("library") },
    { id: "editor", label: "编辑器", action: () => setTab("editor") },
    { id: "progress", label: "创作进度", action: () => setTab("progress") },
    { id: "review", label: "审阅", action: () => setTab("review") },
    { id: "foreshadowing", label: "伏笔看板", action: () => setTab("foreshadowing") },
    { id: "hotspot", label: "热点追踪", action: () => setTab("hotspot") },
    { id: "studio", label: "内容工作室", action: () => setTab("studio") },
    { id: "fanout", label: "多平台分发", action: () => setTab("fanout") },
    { id: "knowledge", label: "知识库", action: () => setTab("knowledge") },
    { id: "publish", label: "发布看板", action: () => setTab("publish") },
    { id: "costs", label: "成本追踪", action: () => setTab("costs") },
    { id: "billing", label: "订阅套餐", action: () => setTab("billing") },
    { id: "prompts", label: "Prompt管理", action: () => setTab("prompts") },
    { id: "chat", label: "AI 对话", action: () => setTab("chat") },
    { id: "dag", label: "工作流编排", action: () => setTab("dag") },
    { id: "versions", label: "版本树", action: () => setTab("versions") },
    { id: "settings", label: "设置", action: () => setTab("settings") },
    { id: "plugins", label: "插件管理", action: () => setTab("plugins") },
    { id: "agents", label: "智能体", action: () => setTab("agents") },
    { id: "collaboration", label: "协作管理", action: () => setTab("collaboration") },
  ];

  const openPalette = () => window.dispatchEvent(new Event("novelcraft:open-command-palette"));
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
        ...
        <NavGroup title="实验室" defaultOpen={['agents','plugins','collaboration','skills'].includes(tab)}>
          ...
          <NavItem icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>}
            label="Skill 中心" active={isActive('skills')} onClick={() => setTab('skills')} />
      ...
      <main className="app-main">
        <header className="topbar">
          <div className="search-box">
            <Search size={16} />
            <input placeholder="搜索项目、章节或命令… (Ctrl+K)" onFocus={openPalette} />
          </div>
