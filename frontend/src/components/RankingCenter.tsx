    ...
    <div className="card">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {sourcesPager.pageData.map(source => {
          ...
          return <div key={source.source_key} style={{
            display: "grid", gridTemplateColumns: "1fr auto auto", alignItems: "center", gap: 12,
            padding: "10px 16px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)",
            background: "var(--bg-elevated)"
          }}>
            <div>
              <strong style={{ fontSize: 14 }}>{source.display_name}</strong>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                {source.source_key} · {source.last_success_at ? new Date(source.last_success_at).toLocaleString("zh-CN") : "未采集"}
              </div>
            </div>
            <span style={{ fontSize: 11, color: healthColor, display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: healthColor }} />{healthLabel}
            </span>
            <button className="btn-sm btn-primary" disabled={!!busy} onClick={() => scan(source)}>
              {busy === `scan:${source.source_key}` ? "采集中…" : "扫榜"}
            </button>
          </div>;
        })}
        <Pagination
                        ...
                        <div>
                              ...
                              return <li key={item.id} style={{ marginBottom: 10, lineHeight: 1.7 }}>
                                ...
                                {item.unified_metrics && (
                                  <div style={{ marginTop: 4, display: "flex", gap: 12 }}>
                                    <small style={{ color: "var(--text-2)", fontWeight: 500 }}>归一化指标：</small>
                                    <small style={{ color: "var(--text-3)" }}>热度指数 {item.unified_metrics.engagement_index ?? "—"}</small>
                                    <small style={{ color: "var(--text-3)" }}>|</small>
                                    <small style={{ color: "var(--text-3)" }}>读者 {item.unified_metrics.readers?.toLocaleString() ?? "—"}</small>
                                    <small style={{ color: "var(--text-3)" }}>|</small>
                                    <small style={{ color: "var(--text-3)" }}>字数 {item.unified_metrics.word_count?.toLocaleString() ?? "—"}</small>
                                    <small style={{ color: "var(--text-3)" }}>|</small>
                                    <small style={{ color: "var(--text-3)" }}>{item.unified_metrics.completion_status || "连载中"}</small>
                                  </div>
                                )}
                          ...
                          {snapshotDetails[snapshot.id]?.latest_analysis && (
                            <div className="card" style={{ marginTop: 14, padding: 16, background: "var(--primary-dim)", borderColor: "var(--border-strong)" }}>
                              ...
                              {snapshotDetails[snapshot.id].latest_analysis?.market_score_avg != null && (
                                <p style={{ fontSize: 13, marginBottom: 4, color: "var(--primary-light)", fontWeight: 600 }}>
                                  市场分均值：{snapshotDetails[snapshot.id].latest_analysis!.market_score_avg}
                                  {(() => {
                                    const t = snapshotDetails[snapshot.id].latest_analysis?.trend;
                                    if (!t) return null;
                                    const dir = t.direction || "stable";
                                    const delta = t.delta != null ? ` (${t.delta > 0 ? "+" : ""}${t.delta})` : "";
                                    return <span style={{ marginLeft: 12, fontWeight: 400 }}>趋势：{dir}{delta}</span>;
                                  })()}
                                </p>
                              )}
