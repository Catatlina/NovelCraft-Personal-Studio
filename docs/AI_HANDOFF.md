...
- 本地证据：章节骨架契约 `8 passed`；前端生产构建通过；一次组合回归中既有融合/骨架相关集成测试因本机 PostgreSQL 未启动而收到注册 `503`，不能标记全量通过。尚未调用新 Provider 样本，尚未提交/推送/部署，不能宣称朱雀95/5/0或三章/20章验收。
- 下一步：完成文档和代码检查后提交、推送、部署；只做一次受控的真实 Provider 单章骨架验证，检查输出结构和人工可写性，再决定是否进行外部朱雀样本；不以“10篇9篇”的他人经验替代本项目证据。
