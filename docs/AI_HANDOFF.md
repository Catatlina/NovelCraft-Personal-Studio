...
- 生产证据：`1a150ff` 已推送并部署；备份 `backups/pre-deploy-1a150ff-20260821-111052.sql.gz`，gzip 校验通过，292M；迁移 `nc_starlume_authoring (head)`；线上 Prompt `1.2.0`；healthz 的数据库/Redis/Worker 均正常，未认证 authoring 路由返回401。
- 当前边界：尚未调用新 Provider 样本，朱雀95/5/0、三章/20章长跑、人工可写性和正式发布闭环仍未验收。下一步只做一次受控的真实 Provider 单章骨架验证，再决定是否进行外部朱雀样本；不以“10篇9篇”的他人经验替代本项目证据。
