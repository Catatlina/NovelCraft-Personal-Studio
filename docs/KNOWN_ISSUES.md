...
# KI-065：章节骨架模式已接线，生产样本待验收（2026-08-21）

- 现状：编辑器本地代码已切换为作者主导的700–1000字章节骨架工作区；整章自动续写不再是默认入口。
- 已修复：骨架独立版本保存、人工修改保存、正文不自动覆盖、当前作品上下文聚合、世界观标题去重。
- 未验收：生产真实 Provider 章节骨架的长度、可读性、连续性和作者可用性；生产浏览器登录态视觉/交互验收。
- 边界：这项改动不等于朱雀 95/5/0、AI披露闭环、三章或20章真实 Provider 长跑验收。
