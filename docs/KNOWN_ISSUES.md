...
## 2026-08-10 KI-040 一致性检查输入/输出接口错位（已修复，待生产复跑）

- 状态：**已接线**（本地修复和回归已通过，尚未部署/复跑）。
...
- 修复：结构化细纲显式 JSON 序列化；检查器兼容 V7 网关使用量信封和旧字符串响应；Provider/坏 JSON/缺字段/非法评分仍 fail-closed。
- 边界：旧章节不回填通过状态；发布后必须从第 1 章重新跑两本各 10 章，并核对一致性调用记录、连续性证据和质量门结果。
