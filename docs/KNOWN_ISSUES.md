...
- 真实生产数据的处理方式：先用 `/api/v1/chapter-scope/scan` dry-run；直接证据充分的记录可安全自动绑定；模糊记录进入 `pending`，由 `/api/v1/chapter-scope/chapters/{chapter_id}/bind` 人工确认；无法判断的记录保持只读，不允许用“唯一项目”这种弱证据静默猜测。
- 这不是正文丢失，也不是数据库清空；是为了防止错误归属污染 Novel Brain 而主动保留的安全边界。当前尚未在生产执行迁移、扫描或人工确认，不能宣称历史归属已验收。
