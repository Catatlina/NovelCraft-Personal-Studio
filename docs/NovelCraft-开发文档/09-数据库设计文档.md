...
pending ──▶ running ──┬──▶ waiting_human ──(确认)──▶ running
                      │
                      ├──▶ failed ──(显式重试)──▶ running          (Provider 调用失败直接失败)
...
**关键验收红线（架构评审 §3）：**
- `pending_provider` 为历史兼容枚举，新业务语义不再使用；Provider 调用失败必须进入 `failed` 并暴露错误。
- `failed` 可单点重跑（不重跑全链），但不得生成占位产物。
...
| reviews.generation_key、generation_batches 四类质量计数 | `nc_sc004_review_gate` | 审核记录幂等；区分已生成、已审核、审核通过和待返工，禁止未审批次伪装成功 |
| generation_batches.start_seq/current_ordinal/quality_status | `nc_sc004_slot_recovery` | 记录批次起始序号、执行中槽位和质量状态；历史批次默认 `legacy_unverified`，禁止冒充已验收 |
...
| workflows.project_id 与项目级唯一索引 | `nc_audit_workflow_scope` | DAG 设计稿按项目隔离；继续使用既有 `definition` JSONB，禁止代码引用不存在的 `config` 列 |
