...
| 三章真实 Provider 长跑 | 未开始 | 生产空副本已执行一次但第1章生成期预算阻断、未落正文；修复已接线待复验 | 清空副本、真实生成、两位盲评 |

### 三章尝试补充

三章验收脚本已在生产空副本启动过一次，但第 1 章生成期预算门禁阻断且未落正文，故“三章真实 Provider 长跑”仍不能标记为已验收。修复已加入生成 Prompt 和回归测试，待部署后单次复验。
