...
# 2026-08-21 `lengdu` 方法清洁室融合矩阵（本地接线）
...
| 原稿与 AI 候选隔离 | 可用 | 仍独立写入 `versions(entity_type=chapter_skeleton)`，正文不变 | 新代码部署后的真实账号回归 |
| 场景链在编辑器可见 | 已接线 | 前端展示场景链、人物变化、伏笔动作、待人工确认事实 | 生产浏览器登录态视觉验收 |
