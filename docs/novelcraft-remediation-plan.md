...
> - ✅ **阶段二 P1 增量 B（P1-T6 导航收拢 / P1-T7 发布闭环 / P1-T9 跨租户矩阵 / P1-T11 后端指标归一）**：已实现并上线（commit 批次 2）。P1-T8 前端单测见 P3/P2 后续批次；P1-T10 DashboardV2 死链因 DashboardV2 整页已在前端审计 T3 删除而 moot。
> - ✅ **阶段三部分（P2-T6 注入清洗 / P2-T7 压测+SLO / P2-T8 注入测试+错误脱敏+日志脱敏 / P2-T10 入参校验+测试）**：已实现（批次 2/3）。P2-T1~T5/T9 见后续批次。
> - ✅ **阶段四 P3-T4（SAST/密钥扫描 CI）**：bandit/semgrep job 已合入（批次 3）。
> - ⏳ **剩余**：P2-T1/T2/T3/T4/T5/T9、P3-T1/T2/T3、P1-T8、P1-T11 前端展示 —— 进行中（批次 4+）。
...
### P1-T6 — 导航收拢（≤8 核心入口 + 实验室）（P1-6） · ✅ 已完成（2026-07-20）
...
### P1-T7 — 发布闭环（Key 校验 + 状态回流）（P1-7） · ✅ 已完成（2026-07-20）
...
### P1-T9 — 跨租户隔离矩阵测试（P1-9 / G4） · ✅ 已完成（2026-07-20，test_authz_matrix.py 13 用例随增量 A 合入）
...
### P1-T10 — DashboardV2 "灵感创作" 死链修复（P1-10） · 🟢 moot（DashboardV2.tsx 已在前端真实性审计 T3 整页删除，无死链残留）
...
### P1-T11 — 扫榜指标归一 + 时序快照（P1-11） · ✅ 后端已完成（2026-07-20），前端展示进行中
...
### P2-T6 — 用户字段注入兜底清洗（P2-7 / Q13） · ✅ 已完成（2026-07-20）
...
### P2-T7 — 真实负载压测 + SLO（P2-8 / Q5(G5) / G10） · ✅ 已完成（2026-07-20，locustfile + docs/slo.md）
...
### P2-T8 — 提示注入测试 + 错误回显脱敏（P2-9 / G7 + P2-10 / G9） · ✅ 已完成（2026-07-20，test_prompt_injection.py + public_message() + 日志脱敏过滤器）
...
### P2-T10 — 长文本/流式资源测试 + 入参校验补齐（G2 / G6） · ✅ 已完成（2026-07-20，入参校验 + test_input_validation.py + test_stream_resources.py + sse_event 抽取）
...
### P3-T4 — SAST / 密钥扫描流水线 + 空态/可达性（P3-4 / G11） · ✅ SAST CI 已完成（2026-07-20，bandit/semgrep job + .bandit）；空态/可达性进行中
