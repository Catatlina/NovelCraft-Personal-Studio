...
### 2026-07-18 前端新 UI 对齐 + AI 失败语义整改（本地验证）

- **前端新 UI 接线**：React 主入口改为真实 API 驱动，覆盖登录/注册/退出、首页、工作台、扫榜/分析、书库、编辑器、热点、发布、成本、Prompt、工作流、设置、Agent 等入口；Dashboard/Settings 文案改为后端返回或待检测，不再静态宣称 Provider 在线。旧 `frontend/public/nc-api.js` 兼容原型脚本已于 2026-07-19 删除，不再作为产品入口或交付证据。
- **后端诚信修复**：`deai_score` 不再在 Provider/输出失败时退回启发式分数；`GET /contents/{id}/deai/score` 失败时返回明确 502；`quick-score` 保留为明示的本地启发式指标。十层分析 `_call_ai` 不再把线程内 Provider 异常包成普通数据并标记 succeeded；`/ranking/analyze` 对 partial 结果按 failed 状态入库，避免失败 AI 层被后续“最新成功分析”读取。
- **门禁校准**：`scripts/verify_ai_truthfulness.py` 补充真实 AI wrapper 识别与确定性统计函数 allowlist，allowlist 仅覆盖本地聚合/路由转发/显式 heuristic endpoint，并附原因；新增 `test_ai_failure_truthfulness_regression.py` 钉住 DeAI 与十层 AI 失败语义。
- **验证**：`python3 scripts/verify_ai_truthfulness.py` 通过；`cd backend && .venv/bin/pytest -q tests/test_ai_failure_truthfulness_regression.py tests/test_truthfulness_gate.py tests/test_v2_bootstrap_wiring.py` → 18 passed；`npm --prefix frontend run build` 通过；`git diff --check` 通过。普通 `bash scripts/ai_development_gate.sh` 的 AST/固定模板/硬编码能力检查通过，但因历史宽泛 placeholder/fallback 扫描仍有警告，需按规范在交付报告解释或用 `GATE_ALLOW_WARNINGS=1` 复跑。
...
- **新版 UI 对账巡检（2026-07-18/19）**：新增 `docs/NovelCraft-开发文档/38-前后端页面接口巡检-20260718.md`。浏览器逐页实测侧边栏 **22/22 页面可打开**；修复概览/工作区/插件占位、编辑器无章节崩溃、版本树/伏笔空白、协作管理缺入口、多平台分发空 content_id、设置页平台连接 label 和成本页 heading。Playwright `set -a; source .env.local; set +a; npm --prefix frontend run test:e2e` 实测 **5 passed**，含真实 Provider 主链②（AI 分析→原创选题→建书入库）；前端 build、AI truthfulness、diff check、允许 warning 的强制 gate 均通过。该结果证明新版 UI 主入口、无 AI 主链和前端真实 Provider 选题建书链已对齐；不替代真实平台回执、7 天调度或 T5 长跑验收。
- **工作台/概览实质接入（2026-07-19）**：按用户反馈重做工作台 `DashboardV2` 的数据层，移除“星渊纪元/赛博长安/今日生成字数”等静态样例，保留原 UI 原型中实用的信息架构（快捷入口、真实书库、榜单快照、AI 选题、系统状态、下一步操作链）。概览保留趋势、题材分布、最近动态、待办和周报导出，改由项目/章节/AI 调用/预算/榜单/选题/analytics 推导；无项目功能页提供“创建项目并继续”真实路径；预算读取与普通账号 admin route 403 均已修正为受控状态。浏览器实测本地登录后工作台/概览/新建作品入口可用；Playwright `set -a; source .env.local; set +a; npm --prefix frontend run test:e2e` 实测 **5 passed**（含真实 Provider 主链②）。
- **发布回执闭环（2026-07-19）**：发布看板改为读取真实项目/书库内容并选择发布对象，不再只靠手填 Content ID；新增 `/api/v1/publish-receipts` 登记人工/平台真实回执（URL、状态、互动数据、收益、备注），同步更新 `publish_records` 与 ROI 看板。验证：TestClient 冒烟显示 records=published、ROI totals 回流；`set -a; source .env.local; set +a; cd backend && NOVELCRAFT_TEST_REDIS_URL=redis://localhost:6379/0 .venv/bin/pytest -q tests/test_publish_center.py` → **5 passed**。该证据证明回执登记/回流闭环可用，不替代真实平台自动发布成功回执。
- **旧原型入口下线（2026-07-19）**：按用户指出“前端仍是假页面”，确认 `/proto.html` 仍是旧静态原型并包含大量样例书名/假按钮；已将 `frontend/proto.html` 替换为真实主应用跳转入口，访问 `/proto.html` 不再展示静态原型数据。真实验收口径统一以 React 主入口和接口/E2E 为准，旧兼容脚本不得作为完成证据。
- **生产前端镜像修复（2026-07-19）**：再次复查用户反馈“还是假接线”后发现 `frontend/Dockerfile` 将 `proto.html` 直接复制为 nginx `index.html`，导致 Docker/线上前端绕过 React 构建产物。已改为 Node 多阶段构建：先执行 `npm ci && npm run build`，再由 `server.mjs` 服务 `/app/dist/` 并代理 `/api` 到后端；新增 `frontend/.dockerignore`，并删除旧 `frontend/public/nc-api.js`。验证：`docker build -t novelcraft-frontend-real-ui:codex frontend` 通过；容器冒烟 `curl http://127.0.0.1:5280/` 返回 React bundle 的 `index.html`（含 `/assets/index-*.js`），不是 `proto.html`，也不含 `nc-api.js`。
- **前端真实接入矩阵（2026-07-19）**：React 左侧新增“工作区”入口，工作区内增加 `IntegrationMatrix`，列出 22 个主要页面/操作与对应 `/api/v1` 接口；GET 类接口会在当前登录态下实际请求并显示“接口可访问/错误”，操作型接口和缺项目/作品/章节上下文的能力显示橙色阻断，不再把缺上下文伪装成成功。浏览器实测注册后进入工作区，矩阵可见，15 个接口可访问，6 个显示真实阻断，页面无“星渊/赛博/青梅/当前原型/ncToast”旧假数据。
- **前端重写接入（2026-07-19，本轮）**：按用户要求“重新开发前端”，确认后端仍可装载 OpenAPI：`openapi_paths=177`、`openapi_operations=186`。`frontend/src/App.tsx` 已替换为 OpenAPI 驱动的新前端外壳：真实工作台、创作主链、扫榜书库、书库编辑、编辑审阅、热点内容、发布回流、知识协作、配置运维、全量接口均从后端读取或调用；`/openapi.json` 已加入 Vite 与生产 `server.mjs` 代理。修复扫榜按钮误接 `/ranking/analyze`（只产十层报告、不产选题）的问题，改接 `/ranking/snapshots/{snapshot_id}/analyze`；修复导入榜单隐式触发不稳定，改为显式“导入榜单”；修复选题建书后不刷新书库。验证：`npm --prefix frontend run build` 通过；`set -a; source .env.local; set +a; npm --prefix frontend run test:e2e` → **5 passed**，含真实 Provider 的“AI 分析→原创选题→建书入库”；`docker build -t novelcraft-frontend-real-ui:codex frontend` 通过；容器冒烟返回 React assets 且 `has_proto_static=False`；`python3 scripts/verify_ai_truthfulness.py` 通过；`bash scripts/ai_development_gate.sh` 正常模式仍因历史宽泛 fallback/placeholder 扫描警告失败，`GATE_ALLOW_WARNINGS=1 bash scripts/ai_development_gate.sh` 可通过但需交付报告说明警告。
