...
| 阶段 | 完成度 | 说明 |
|---|---|---|
| M1 地基+MVP | ~70% | Auth 接入了核心端点(5 tests)，PG 28表就绪，DeepSeek gateway 可用 |
| M2 小说引擎 | ~55% | 上下文装配/伏笔/摘要代码存在，缺 API Key 无法端到端验证 |
| M3 内容工作室 | ~25% | 短篇/自媒体/知识库后端骨架存在，热点是 LLM 幻觉 |
| M4 发布出海 | ~15% | 发布网关只插 DB 记录，海外翻译未验证 |
| M5 协作多端 | ~10% | 协作修了表结构，PWA 只有 manifest |
